...
void Caffe::set_cublas_stream(cudaStream_t stream_id) {
  if (Get().cublas_handle_) {
    CUBLAS_CHECK(cublasSetStream(Get().cublas_handle_, stream_id));
  } else {
    LOG(ERROR) << "cuBLAS not available. Skipping setting its stream.";
  }
}
