...
def _Net_forward(self, blobs=None, start=None, end=None, **kwargs):
    ...
    if kwargs:
     ...
     #       print in_, blob.shape, self.blobs[in_].shape
        for in_, blob in kwargs.iteritems():
            if blob.shape[0] != self.blobs[in_].num:
                raise Exception('Input is not batch sized : ' + str(blob.shape[0]) + " " + in_ + str(self.blobs[in_].num) + " " + str(self.blobs[in_].shape[0]))
